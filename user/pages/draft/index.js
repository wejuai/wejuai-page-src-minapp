...
Page({
  ...
  getEssayList(obj) {
    ...
    app.REQUEST(app.globalData.API.articleDraftList, 'GET', { page: current, size: essay.size, titleStr, direction })
    .then(res => {
      if(res.statusCode == 200) {
        let list = res.data.content;

        if(current > 0) {
          list = [ ...that.data.essay.list, ...list ]
        }
  ...
  getGroupList(obj) {
    ...
    app.REQUEST(app.globalData.API.rewardSubmissionDraft, 'GET', { page: current, size: group.size })
    .then(res => {
      if(res.statusCode == 200) {
        let list = res.data.content;

        if(current > 0) {
          list = [ ...that.data.group.list, ...list ]
        }
