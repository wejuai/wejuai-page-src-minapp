...
Page({
  ...
  getList(obj) {
    ...
    let params = { page: current, size: attention.size };
    if(searchText) params.titleStr = searchText;

    app.REQUEST( app.globalData.API.attentionList, 'GET', params, false, { showLoading: true } )
    .then(res => {
  ...
  toMessage(e) {
    ...
    app.setPageParams(url, {
      avatar: info.headImage, userId: info.id, nickname: info.nickName
    })
    wx.navigateTo({ url: `${url}?name=${info.nickName}` })
  },
  // 跳转 用户详情
  toOther(e) {
